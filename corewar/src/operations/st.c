...
void op_st(process_t *process, vm_t *vm)
{
    ...
    if (process->instruction->arg_types[1] == 1 && (nb > 15 || nb < 0)) {
        process->carry = 0;
        return;
    }
    if (process->instruction->arg_types[0] != 1 && process->instruction->arg_types[1] == 2 && (registre > 15 || registre < 0))
        process->carry = 0;
    else {
        if (process->instruction->arg_types[1] == 3)
            int_to_mem(vm->mem, pc + nb % IDX_MOD, process->reg[registre]);
