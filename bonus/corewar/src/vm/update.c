...
{
    void (*instruct[])(process_t *, vm_t *) = {op_live, op_ld, op_st, op_add,
    op_sub, op_and, op_or, op_xor, op_zjmp, op_ldi, op_sti, op_fork, op_lld,
...
void update(vm_t *vm)
{
    ...
    for (list_t *champ = vm->champ; champ != NULL; champ = champ->next) {
        ...
        for (list_t *process = champ_temp->process; process != NULL;
